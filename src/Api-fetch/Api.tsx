...
const Api = (props: InfoboxProps) => {
    ...
    useEffect( ()=>{
        ...
        setSearch (response.results[0]) 
            ...
            const d = await fetch(`https://api.themoviedb.org/3/movie/${id}?language=en-US`, options)
        ...
        const v = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US`, options)
            const response2 = await v.json()

            setVideo (response2.results[0])
            console.log ("Video", response2)

        const c = await fetch(`https://api.themoviedb.org/3/movie/${id}/credits?language=en-US`, options)
    ...
    return (
        <Infobox title={search.title} date={search.release_date} rating={search.vote_average.toFixed(1)} description={search.overview} cast={cast} duration={duration.runtime+"m"} streams={{
            disney: false,
            netflix: true,
            hulu: true,
            siminn: true,
